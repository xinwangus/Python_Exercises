...
class Cal24op():
    ...
    def cal(self):
        ...
        # objects
            ...
            self.valid = self.l.valid and self.r.valid
            if self.valid:
                 if self.l.result >= self.r.result and \
                    self.r.result != 0 and \
                    self.l.result % self.r.result == 0:
                        self.result = int(self.l.result / self.r.result)
                        self.print_result = "(" + self.l.print_result + \
                                            " / " + self.r.print_result + ")"
                 elif self.r.result >= self.l.result and \
                      self.l.result != 0 and \
                      self.r.result % self.l.result == 0:
                        self.result = int(self.r.result / self.l.result)
                        self.print_result = "(" + self.r.print_result + \
                                            " / " + self.l.print_result + ")"
                 else:
                     self.valid = False
        else:
